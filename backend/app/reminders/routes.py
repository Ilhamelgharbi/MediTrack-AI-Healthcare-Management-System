...
def create_reminder_schedule(
    ...
    try:
        ...
        # Convert reminder_times JSON to list
        import json
        schedule_dict = {
            "id": schedule.id,
...
def get_reminder_schedules(
    ...
    # Convert reminder_times JSON to list for each schedule
    import json
    result = []
...
def generate_reminders(
    ...
    schedule = service.get_reminder_schedule(
        patient_id=current_user.id,
        patient_medication_id=db.query(ReminderSchedule).get(schedule_id).patient_medication_id
    )
